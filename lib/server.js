...
exports.run = function(config) {
    ...
    c2s.on("inStanza", function(client, stanza) {
        ...
        c2s.route(stanza); // Allow for the incoming (from C2SStream) stanzas to be routed... we first route to the S2S component, which will then maybe forward to the C2SServer.. and this one may push it down to the right C2SStream
    });
    ...
    c2s.on("recipient_offline", function(stanza) {
        RecipientOffline.storeOfflineMessage(stanza);
    });
