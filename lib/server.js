...
exports.run = function(config, ready) {
    ...
    Logger.configure(c2s, config.logger);
    Offline.configure(c2s, config.offline);
    SoftwareVersion.configure(c2s, config.offine);
    Presence.configure(c2s, config.presence);
    Roster.configure(c2s, config.roster);
    DiscoInfo.configure(c2s, config.disco);
    VCard.configure(c2s, config.vcard);
    Websocket.configure(c2s, config.websocket);
